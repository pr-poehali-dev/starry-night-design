      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((stat, i) => (
            <div
              key={i}
              className="text-center p-8 rounded-2xl bg-background border border-border hover:border-amber-500/40 transition-all duration-300 slide-up"
              style={{ animationDelay: `${i * 0.1}s` }}
            >
              <Icon name={stat.icon as "BookOpen"} size={32} className="text-amber-500 mx-auto mb-4" />
              <div className="text-3xl font-bold text-foreground font-orbitron mb-2">{stat.value}</div>
